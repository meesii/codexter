    ...
    Future<void> _importFromCodex() async {
        ...
        try {
            ...
            for (final item in scanned) {
                final existing = widget.appState.mcps
                    .firstWhereOrNull((mcp) => mcp.name == item.name);
                if (existing != null) continue;
                await widget.appState.saveMcp(DownstreamMcpEntry(
                    name: item.name,
                        ...
                        children: [
                            ...
                            Row(
                                children: [
                                    Expanded(
                                        child: Text(
                                            '工具',
                                            style: AppTones.title(Theme.of(context), size: 13),
                                        ),
                                    ),
                                    Text(
                                                ...
                                                child: TextField(
                                                    ...
                                                    features: const [
                                                        InputFeature.leading(
                                                            Icon(BootstrapIcons.search, size: 12),
                                                        ),
                                                    ],
                            ...
                            if (allTools.isEmpty)
                                ...
                                ...tools.map(
                                    (tool) => _McpToolItem(
                                        tool: tool,
                                        onTap: () => _openToolDetails(context, tool),
                                    ),
                                ),
                        ],
                    );
            ...
            actions: (dialogContext) => [
                ...
                Button(
                    ...
                    onPressed: () async {
                        ...
                        AppToast.success(
                            dialogContext,
                            entry == null ? '下游 MCP 已添加' : '下游 MCP 已保存',
                        );
                    },
                ...
                children: [
                    ...
                    Wrap(
                        ...
                        children: [
                            _McpSummaryMeta(
                                label: '启动',
                                value: '${(client?.startupTimeoutMs ?? entry.startupTimeoutMs ?? defaultStartupTimeoutMs) ~/ 1000}s',
                            ),
                            _McpSummaryMeta(
                                label: '工具',
                                value: '${(client?.toolTimeoutMs ?? entry.toolTimeoutMs ?? defaultToolTimeoutMs) ~/ 1000}s',
                            ),
