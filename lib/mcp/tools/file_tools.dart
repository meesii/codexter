...
class FileTools {
  ...
  static void register(ToolRegistry registry, ToolContext context) {
    ...
    registry.register(_readSchema, (raw) async {
      ...
      return ToolResult.text(
        buffer.toString(),
        structured: {'files': results, 'truncated': targets.length > maxReadFiles},
      );
    });
    ...
    registry.register(_readImageSchema, (raw) async {
      ...
      final canPassthrough =
          originalSize <= imagePassthroughBytes && originalLongEdge <= maxLongEdge;
      if (canPassthrough) {
        return _imageResult(
          relative: relative,
          bytes: originalBytes,
    ...
    registry.register(_lsSchema, (raw) async {
      ...
      return ToolResult.text(
        buffer.toString(),
        structured: {'path': relative, 'count': entries.length, 'entries': entries},
      );
    });
  }
  ...
  static ToolResult _imageResult({
    ...
    return ToolResult.image(data: base64Encode(bytes), mimeType: mimeType, structured: structured);
  }
  ...
  static const _readSchema = ToolSchema(
    ...
    meta: {'openai/toolInvocation/invoking': '正在读取文件…', 'openai/toolInvocation/invoked': '已读取文件'},
  );
  ...
  static const _readImageSchema = ToolSchema(
    ...
    inputSchema: {
      'type': 'object',
      'properties': {
        'path': {'type': 'string', 'description': 'Image path relative to project root'},
        'process': {
          'type': 'string',
          'enum': ['auto', 'none'],
          'default': 'auto',
          'description':
              'auto resizes/compresses large images; none returns the original image subject to output limits',
        },
        'max_long_edge': {
          'type': 'integer',
          'minimum': 256,
    ...
    annotations: ToolAnnotations.readOnly,
    meta: {'openai/toolInvocation/invoking': '正在读取图片…', 'openai/toolInvocation/invoked': '已读取图片'},
  );
  ...
  static const _applyPatchSchema = ToolSchema(
    ...
    meta: {'openai/toolInvocation/invoking': '正在应用补丁…', 'openai/toolInvocation/invoked': '已应用补丁'},
  );
  ...
  static const _lsSchema = ToolSchema(
    ...
    annotations: ToolAnnotations.readOnly,
    meta: {'openai/toolInvocation/invoking': '正在列出目录…', 'openai/toolInvocation/invoked': '已列出目录'},
  );
}
